...
export function LearnMore({ cards }: { cards: Card[] }) {
  return (
    <div className="relative bg-white" id="features">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 max-w-[75rem] mx-auto md:w-full pt-16 pb-24 w-80 sm:px-4 px-0">
        <div>
          ...
          <h2 className="text-xl/[1.625rem] font-semibold text-[#131316] tracking-tight mt-2 mb-3">
            Controlled access to private project files
          </h2>
          <p className="text-[0.8125rem]/5 text-[#5E5F6E]">
            Setup Github actions workflow, control access using Clerk and share
            securely
          </p>
        </div>
        {cards.map((card) => (
